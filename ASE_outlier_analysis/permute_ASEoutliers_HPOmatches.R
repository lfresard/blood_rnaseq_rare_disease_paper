...
## Read in all topN - assumes data files have been generated
load(paste0(data_dir, 'ASE_top20_HPO_matches.RData'))

allHpoMatches20 = as.data.frame(rbind(hpoMatchesOutliers %>% mutate(Filter='JustASE'),
                                      hpoMatchesOutliersPli %>% mutate(Filter='pLI'),
                                      hpoMatchesOutliersRV %>% mutate(Filter='RV'),
                                      hpoMatchesOutliersCRV %>% mutate(Filter='Conserved_RV'))) %>%
  mutate(N=20)

load(paste0(data_dir, 'ASE_top10_HPO_matches.RData'))

allHpoMatches10 = as.data.frame(rbind(hpoMatchesOutliers %>% mutate(Filter='JustASE'),
                                      hpoMatchesOutliersPli %>% mutate(Filter='pLI'),
                                      hpoMatchesOutliersRV %>% mutate(Filter='RV'),
                                      hpoMatchesOutliersCRV %>% mutate(Filter='Conserved_RV'))) %>%
  mutate(N=10)

load(paste0(data_dir, 'ASE_top50_HPO_matches.RData'))

allHpoMatches50 = as.data.frame(rbind(hpoMatchesOutliers %>% mutate(Filter='JustASE'),
                                      hpoMatchesOutliersPli %>% mutate(Filter='pLI'),
                                      hpoMatchesOutliersRV %>% mutate(Filter='RV'),
                                      hpoMatchesOutliersCRV %>% mutate(Filter='Conserved_RV'))) %>%
  mutate(N=50)

load(paste0(data_dir, 'ASE_top100_HPO_matches.RData'))

allHpoMatches100 = as.data.frame(rbind(hpoMatchesOutliers %>% mutate(Filter='JustASE'),
                                       hpoMatchesOutliersPli %>% mutate(Filter='pLI'),
                                       hpoMatchesOutliersRV %>% mutate(Filter='RV'),
                                       hpoMatchesOutliersCRV %>% mutate(Filter='Conserved_RV'))) %>%
  mutate(N=100)
...
allHpoMatches = rbind(allHpoMatches10,allHpoMatches20,allHpoMatches50,allHpoMatches100)

# Write out all combined for use in future
write.table(allHpoMatches,file=paste0(data_dir, 'allHpoMatches.txt'),sep='\t',quote=F,row.names=F)

allHpoMatches = fread(paste0(data_dir, 'allHpoMatches.txt'))

allHpoMatches$Filter = factor(allHpoMatches$Filter,levels=c('JustASE','pLI','RV','Conserved_RV'))

matchPlot = ggplot(allHpoMatches,aes(x=Filter,y=NumMatch,Group=Category)) +
  geom_boxplot(aes(fill=Category)) + theme_bw() + 
  ylab('Number HPO matches out of top N genes per ind') +
  scale_fill_manual(values=c('grey','black')) + facet_grid(.~N)

save(allHpoMatches,matchPlot,file=paste0(data_dir, 'hpoMatchASEFilterPlot.RData'))
